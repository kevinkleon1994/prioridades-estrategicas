...
let user=null,records=[],tasks=[],evidences=[],systemUsers=[],churchStats=[],selectedChurch="Todas",selectedYear="2026",selectedMonth="Todos",deferredPrompt=null,currentPriority="Identidade",selectedCriterionCode=null,criteriaStatus="Todos";
...
async function loadData(){ $("refreshIcon").classList.add("spin"); try{const res=await jsonp(endpoint());const rows=Array.isArray(res)?res:res.data;if(!Array.isArray(rows))throw Error();records=rows.map(normalize);localStorage.setItem("v5cache",JSON.stringify(records));try{const statsResult=await jsonp(endpoint(),"listChurchStats",{});churchStats=Array.isArray(statsResult)?statsResult:(statsResult.data||[]);localStorage.setItem("v81churchStats",JSON.stringify(churchStats))}catch(_statsError){churchStats=JSON.parse(localStorage.getItem("v81churchStats")||"[]")}$("syncBadge").innerHTML="<i></i>Google Sheets conectado"}catch(e){records=JSON.parse(localStorage.getItem("v5cache")||"[]");churchStats=JSON.parse(localStorage.getItem("v81churchStats")||"[]");$("syncBadge").innerHTML="<i style='background:#ffb800'></i>Modo offline"}finally{$("refreshIcon").classList.remove("spin");renderAll()}}
...
function renderAll(){$("contextText").textContent=`${selectedChurch==="Todas"?"Distrito completo":selectedChurch} · ${selectedYear}`;$("lastUpdate").textContent="Última atualização: "+new Date().toLocaleString("pt-BR");renderMemberStats();renderDashboard();renderPriorities();renderPlanner();renderTimeline();renderEvidence();renderAdmin()}
function selectedChurchStats(){
  if(selectedChurch==="Todas"){
    return churchStats.reduce((a,s)=>({total:a.total+Number(s.total_membros||s.total||0),ativos:a.ativos+Number(s.membros_ativos||s.ativos||0)}),{total:0,ativos:0});
  }
  const s=churchStats.find(x=>String(x.igreja)===String(selectedChurch))||{};
  return {total:Number(s.total_membros||s.total||0),ativos:Number(s.membros_ativos||s.ativos||0)};
}
function renderMemberStats(){const s=selectedChurchStats();$("totalMembers").textContent=s.total;$("activeMembers").textContent=s.ativos;$("rescueMembers").textContent=Math.max(0,s.total-s.ativos);document.querySelectorAll("[data-member-edit]").forEach(b=>b.disabled=selectedChurch==="Todas"||!canEditChurch(selectedChurch))}
function openMembersModal(){if(selectedChurch==="Todas")return toast("Selecione uma igreja para editar os membros.");if(!canEditChurch(selectedChurch))return;const s=selectedChurchStats();$("membersChurchName").textContent=selectedChurch;$("membersTotalInput").value=s.total;$("membersActiveInput").value=s.ativos;updateRescuePreview();$("membersModal").classList.add("open")}
function updateRescuePreview(){$("membersRescuePreview").textContent=Math.max(0,(Number($("membersTotalInput").value)||0)-(Number($("membersActiveInput").value)||0))}
async function saveMemberStats(){if(selectedChurch==="Todas"||!canEditChurch(selectedChurch))return;const total=Number($("membersTotalInput").value)||0,ativos=Number($("membersActiveInput").value)||0;if(ativos>total)return toast("Membros ativos não pode ser maior que o total.");const body=new URLSearchParams({action:"saveChurchStats",igreja:selectedChurch,total_membros:String(total),membros_ativos:String(ativos),usuario:user.email});$("saveMembersButton").disabled=true;try{await fetch(endpoint(),{method:"POST",mode:"no-cors",body});const idx=churchStats.findIndex(x=>x.igreja===selectedChurch),row={igreja:selectedChurch,total_membros:total,membros_ativos:ativos};if(idx>=0)churchStats[idx]={...churchStats[idx],...row};else churchStats.push(row);localStorage.setItem("v81churchStats",JSON.stringify(churchStats));renderMemberStats();$("membersModal").classList.remove("open");toast("Quantidade de membros salva.")}finally{$("saveMembersButton").disabled=false}}
